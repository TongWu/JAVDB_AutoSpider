# IMP-ADR010-01: ADR-010 Phase 1 — D1 Access Port Core
...
**Architecture:** `D1AccessPort` owns HTTP POSTs, retry/backoff parity, schema metadata caching, and summary metrics. `D1Connection` remains the sqlite3-compatible facade. Recovery outbox files and CLI are introduced as inert tooling; queueing/replay success semantics are left to IMP-ADR010-02.
...
Create the module with `RecoveryPolicy`, `RecoveryEvent`, `append_event`, `load_latest_events`, `pending_by_ordering_key`, and `compact_replayed` exactly as specified in [ADR-010](ADR-010-d1-access-port.md) D5.
