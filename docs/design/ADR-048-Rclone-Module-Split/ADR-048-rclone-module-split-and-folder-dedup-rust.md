...
| Field       | Value                                                                 |
| ----------- | --------------------------------------------------------------------- |
| **Status**  | Completed (2026-06-19) — all 3 phases shipped; see [IMP-ADR048-01](IMP-ADR048-01-module-split-and-rust-folder-dedup.md) |
| **Date**    | 2026-06-13                                                            |
| **Authors** | Ted                                                                   |
...
- 2026-06-19: Phase 2 done. Fixed Rust `parse_lsjson_for_year` for the 3-level `<actor>/<movie_code>/<sensor-subtitle>` layout (Rust `#[test]`s green); routed `scan.py` `get_year_folders`/`get_actor_folders`/`get_all_movie_folders_for_year` through Rust `parse_lsd_output`/`parse_lsjson_for_year` with the pure-Python Best-Effort fallback (D7) on `ImportError`; pinned by `tests/unit/test_rclone_scan_parity.py`. **Divergence:** `group_folders_by_movie_code` was NOT routed through Rust `group_by_movie_code` — it groups `FolderInfo` dataclasses (Rust operates on plain dicts), so routing would force a lossy round-trip for a trivial non-hot-path grouping; left pure Python. The Python folder-dedup cascade + `SIZE_THRESHOLD_RATIO` are untouched by Phase 2 and remain live until Phase 3.
- 2026-06-19: Phase 3 done — ADR Completed. Ported the folder-dedup keep/delete *decision* to Rust (`dedup_ops.rs` `analyze_folder_dedup`), enforcing the D5 invariants fail-closed (partition, never-empty-keep, single sensor winner, strict 1.30× size exception); Phase 3a proved Rust == the Python cascade across a 20k-input brute force before deletion (ADR-041 D5a order). Phase 3b removed the Python cascade (`_process_*`/`_apply_sensor_priority`) and added the Rust-Required chokepoint guard (`_require_rust_dedup`, clear `RuntimeError` naming the wheel build); `execute_deletions`/`rclone_purge` + reason-string formatting stay Python (D4). Migration-time parity retired; its diverse fixtures live on as a frozen golden (`tests/unit/test_rclone_dedup_golden.py`). CONTEXT.md gained the Rust-Required extension term; ADR-041 Status Log back-references this extension (D6).
