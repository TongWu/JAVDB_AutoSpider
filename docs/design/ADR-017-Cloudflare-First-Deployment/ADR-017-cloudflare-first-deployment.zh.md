...
将全栈应用（Vue 3 前端 + API 后端）部署在 **Cloudflare Pages** 上作为统一项目，同时**保留现有 Docker 部署**作为替代方案。

### 架构

```
Cloudflare Pages (javdb-autospider-web 仓库)
├── Vue 3 SPA (静态资源, 全球 CDN)
└── Pages Functions (/api/*)
    ├── Hono 框架 (TypeScript)
    ├── D1 绑定 (原生, 零延迟)
...
#### D3: `javdb-autospider-web` 从纯前端演进为全栈 Pages 项目

按 ADR-008，前端在独立仓库中。本 ADR 扩展该仓库以包含 Cloudflare Pages Functions（Hono API），使其成为全栈项目：
...
| 维度 | Cloudflare Pages | Docker |
| ---- | ---------------- | ------ |
| 前端 | Vue SPA (CDN) | nginx 容器 |
