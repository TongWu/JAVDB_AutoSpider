...
**Architecture:** `apps.cli.notify.email` becomes the real CLI adapter. `javdb.integrations.notify.email` becomes a package with `options.py`, `result.py`, `service.py`, `log_analysis.py`, `report_builder.py`, and `delivery.py`. A bake wrapper keeps selected legacy exports until IMP-ADR015-07.

**Tech Stack:** Python 3.11, dataclasses, argparse, pytest, smtplib/email stdlib, existing logging/config/storage helpers.

**Source spec:** [ADR-015](ADR-015-integrations-interface-boundary.md), D1-D6, D8, and D10.
...
IMP-ADR015-07 after the bake window.
"""
...
wrapper. IMP-ADR015-07 removes them.
