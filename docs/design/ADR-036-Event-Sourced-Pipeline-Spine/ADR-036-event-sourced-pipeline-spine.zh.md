...
- [ADR-033](../_archive/ADR-033-Media-Closed-Loop/ADR-033-media-closed-loop.md)（媒体闭环）不得不**给 uploader 插桩**（加种时写）并**从 cleanup 步骤 push**（完成）才能得知种子的命运。
...
| Phase 4 — 操作员审计日志 | [IMP-ADR036-04](IMP-ADR036-04-operator-action-audit-log.md) | 兄弟表 `OperatorAuditEvent` append-only（actor/action/target/source）；在 web/worker/CLI 变更点 best-effort emit；管理员只读 API + Web Audit Log 视图 | 修改 PipelineEvent；管道实体生命周期事件 |
...
- 2026-06-13：新增 Phase 4（IMP-ADR036-04，操作员/Web 控制台动作审计日志），以填补脊柱只记录管道实体生命周期事件、缺少 actor 维度的缺口。操作员变更落入兄弟表 append-only `OperatorAuditEvent`（best-effort），不动 PipelineEvent，也不动保留的 Phase-3 strangler 槽位。
