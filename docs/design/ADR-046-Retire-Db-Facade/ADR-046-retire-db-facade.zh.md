...
- 2026-06-02：Proposed。源自 2026-05-29 架构评审（候选 C），经 2026-06-02 复验后切分：60 个公开 `db_*` 函数仍在、repo 是浅转发、`session_id` 从 `_db_session.py` 的进程级全局解析。选定接缝：单类 + 构造绑定 `session_id` + 运行时写守卫（D2/D3）。Phase 1（IMP-ADR046-01）仅覆盖 History 写接缝；全局 session 机制在后续阶段（Operations/Stats 迁移后）删除（2026-06-03 重界定为专门的 **Phase 5**——见路线图与下方 2026-06-03 状态日志条目）。
