# IMP-033: ADR-015 Phase 4 - Rclone Manager Split
...
**Architecture:** `javdb.integrations.rclone.manager` becomes a package. `apps.cli.rclone.manager` owns CLI parsing and exit-code mapping. The package keeps a bake wrapper that re-exports selected legacy helpers until IMP-034 removes it.

**Tech Stack:** Python 3.11, dataclasses, argparse, pytest, existing rclone helper/manager logic.

**Source spec:** [ADR-015](../adr/ADR-015-integrations-interface-boundary.md), D1-D6 and D8-D9.
...
wrapper can support legacy tests. These are removed in IMP-034.
