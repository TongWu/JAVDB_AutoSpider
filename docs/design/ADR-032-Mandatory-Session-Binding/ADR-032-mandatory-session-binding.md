...
**D1. Make `session_id` mandatory — remove the global fallback (Phase 1).** Drop the `_SESSION_ID_SENTINEL` default from the operations functions and the two history-batch functions; require `session_id` explicitly. A caller that omits it now raises instead of silently writing an untagged row — the intended hardening (ADR-005 Alternative-3). Per-method binding is preserved.
