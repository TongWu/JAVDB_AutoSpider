...
**D8. 数据模型:一张基线表;不另起告警表。** `ParseFieldHealth`（per `page_type × field`:滚动基线填充率、上次填充率、last_observed_at、样本数）是 D1-canonical 基线存储。漂移事件复用 `OpsIncidents`（D6）。
...
| 阶段 | IMP | 交付内容 | 推迟内容 |
| --- | --- | --- | --- |
| Phase 1 — 搭车 + 门控 | [IMP-ADR035-01](IMP-ADR035-01-piggyback-and-gate.md) | `parse_contract`;`field_health` 每次 run 遥测;`detectors`;`ParseFieldHealth` 基线;daily run 上的分级动作（关键门控 + 软 incident）;`site_drift` incident 类型 | 独立 canary;web/AI 面 |
...
- 2026-05-31: 第一期([IMP-ADR035-01](IMP-ADR035-01-piggyback-and-gate.md))
  ...
  fail-open 放行),以及 `apps.cli.ops.sentinel`。偏离计划之处见 IMP 的
  "As-Built Notes"。后续待办:将 D1 迁移应用到生产;为 API 提交路径加闸门。
  第二、三期仍待定,故伞型 ADR 维持 Proposed。
