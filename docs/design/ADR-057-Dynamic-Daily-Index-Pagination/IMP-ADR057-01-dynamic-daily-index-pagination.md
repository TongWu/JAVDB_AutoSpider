...
**Status:** Completed — delivered in [#268](https://github.com/TongWu/JAVDB_AutoSpider_CICD/pull/268), merged 2026-08-09.
...
Shipped as planned, with five amendments made during implementation and recorded inline
below and in the ADR: the unreadable-page budget (D5), the cap-versus-floor distinction
and stop-reason precedence (D6), the dedicated `SPIDER_STAT_PAGE_SCAN_STOP_REASON` line
rather than a suffix on `SPIDER_STAT_PAGES` (D9), `proxies-exhausted` added to the
stop-reason vocabulary, and `ReportSessions.EndPage` taking the effective end page.

Two things the plan did not anticipate, both surfaced by review and both now covered by
regression tests:

- **The dead-proxy-pool path.** `fetch_all_index_pages_parallel` has three separate submit
  sites (deferred fixed range, dynamic extension, `--all` sliding window). Closing only
  some of them on `all_proxies_banned` moved the failure rather than removing it — hang,
  then hang again, then `RuntimeError: Cannot submit after mark_done()`, then a silent
  truncation with no stop reason. All three are now gated on one flag.
- **The probe asserting more than its data supports.** `index_probe.py` repeatedly
  reported a conclusion it had not measured — contiguity over a fetch gap, date ordering
  from an empty sample, `fresh_missed: 0` over a tail it never fetched. Its invariant is
  now explicit: a verdict is only positive when the rule reached a decision *and* every
  page it needed was read.

Also settled here: `PAGE_END`'s default was split (runtime 20, generator 10). Since the
value became the scan floor, both are now 10, with a test asserting they stay equal.
...
- [x] **Step 1 — failing test.** Create `tests/unit/test_index_selection_freshness.py` asserting that `count_new_release_entries` counts entries carrying either date badge, in both locales, ignores entries with only magnet badges, counts an entry with no `video_code` (selection skips those, the freshness signal must not), and returns 0 for a page with `has_movie_list=False`.
- [x] **Step 2 — run it, expect `ImportError`.**
- [x] **Step 3 — implement.** `def count_new_release_entries(page_result) -> int:` returning `sum(1 for e in page_result.movies if _has_release_date(e.tags))`, guarded by `if page_result is None or not page_result.has_movie_list: return 0`.
- [x] **Step 4 — green.** `pytest tests/unit/test_index_selection_freshness.py -v`
...
- [x] **Step 1 — failing tests** covering:
...
- [x] **Step 4 — green + full unit run** for the new file.
...
- [x] **Step 1 — failing test** in `tests/unit/test_page_scan_policy.py`: importing the three names from `javdb.spider.runtime.config` yields the documented defaults (`True`, `30`, `2`).
- [x] **Step 2 — implement** `PAGE_SCAN_DYNAMIC = cfg('PAGE_SCAN_DYNAMIC', True)`, `PAGE_SCAN_MAX = cfg('PAGE_SCAN_MAX', 30)`, `PAGE_SCAN_STOP_AFTER = cfg('PAGE_SCAN_STOP_AFTER', 2)`.
- [x] **Step 3 — generator rows** in the `SPIDER CONFIGURATION` section, using `get_env_bool` / `get_env_int`.
- [x] **Step 4 — `config.py.example`** entries with comments explaining floor / cap / K.
...
- [x] **Step 1 — failing test** in `tests/unit/test_index_dynamic_pagination.py`: a fake `fetch_index_page_with_fallback` serving pages whose fresh counts are `[40, 40, 0, 0]` with `end_page=2` must fetch pages 1–4 and stop (extension + K=2), while the same fake with `PAGE_SCAN_DYNAMIC=False` fetches exactly pages 1–2.
- [x] **Step 2 — run, expect the dynamic case to stop at page 2.**
- [x] **Step 3 — implement.** Build the policy once before the loop (enabled iff `not parse_all and custom_url is None and not ignore_release_date and not IGNORE_RELEASE_DATE_FILTER`); call `policy.observe(...)` right after `parse_index_page` and **before** `filter_blacklisted_families` (D2); replace the hard break with `if not parse_all and not policy.should_continue_after(page_num): break`. Feed `fresh=None` on the fetch-failure / no-movie-list branches (lines ~164–180) so D5 holds, and `end_of_content=True` on the valid-empty branch (~line 160).
- [x] **Step 4 — green.**
...
- [x] **Step 4 — green**, plus `pytest tests/unit/test_index_parallel.py -v` to prove the existing window/stop tests still pass.
...
- [x] **Step 1 — failing test**: `generate_summary_report(..., effective_end_page=14, page_scan_stop_reason='cap')` prints `SPIDER_STAT_PAGES=1-14` plus `SPIDER_STAT_PAGE_SCAN_STOP_REASON=cap`, and the summary pair shows the cap.
- [x] **Step 2 — implement.** Both index paths return `effective_end_page` and `page_scan_stop_reason`; `run_service` forwards them; `report.py` prefers the effective value over the configured one and appends the reason. Log a `WARNING` when the reason is `cap`.
- [x] **Step 3 — green.**
...
- [x] Add `VAR_PAGE_SCAN_DYNAMIC`, `VAR_PAGE_SCAN_MAX`, `VAR_PAGE_SCAN_STOP_AFTER` with the ADR defaults as `||` fallbacks.
- [x] Check the other workflows that render `config.py` for a daily-mode spider run and wire the same three where they apply.
- [x] **Verify:** `python3 -c "import yaml; yaml.safe_load(open('.github/workflows/DailyIngestion.yml'))"`.

### Task 8: Documentation

- [x] `docs/handbook/en/self-hoster/configuration.md` — the three options next to `PAGE_START` / `PAGE_END`, explaining floor / extension / cap and when the extension is inert (D7).
- [x] `docs/handbook/en/self-hoster/github-actions-setup.md` — the three repository variables.
- [x] Mirror both into `docs/handbook/zh/...` in the same commit (code blocks, env names and defaults verbatim).
- [x] **Verify:** `grep -rn "PAGE_SCAN" docs/handbook/` lists en and zh symmetrically.

### Task 9: Full verification

- [x] `python3 -m pytest tests/unit -q` (or the impact-selected subset) — green. CI's impact-selected run is the authority; a full local run is blocked by missing Rust/FastAPI deps, whose failures were verified identical against a clean `origin/main` worktree.
- [x] `python3 -m pytest tests/smoke -q` — 206 passed. The two failures (`test_mcp_server`, `test_reconcile_cli`) reproduce on clean `main` and are the same local dependency gaps.
- [ ] Dry-run the daily spider path locally: `python3 -m apps.cli.spider --dry-run --start-page 1 --end-page 2`, confirming the log reports an effective end page and a stop reason. **Not performed** — the step was conditional on a local config, and the dev container has no `config.py` and no route to javdb. The reporting path is covered by unit tests driving `generate_summary_report` and both fetch paths with fake fetches; an end-to-end confirmation still awaits the first live daily run under `PAGE_SCAN_DYNAMIC=True`.
- [x] Re-read the diff for stray edits; confirm no production behaviour changes when `PAGE_SCAN_DYNAMIC=False`.
