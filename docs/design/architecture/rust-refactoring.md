...
> ⚠️ **2026-06-02 更新（先读这段）**
>
> 下面的正文是 **2026-02 / 03 的历史快照**，其中关于 **“HTTP 请求处理 (requester) 已用 Rust 完成”的说法是错误的**，请以本横幅为准：
>
> - **HTTP fetch 一直是纯 Python**（`javdb/infra/request.py` + `javdb/spider/fetch/fetch_engine.py`，用 `curl_cffi` 做 TLS/JA3 指纹来对抗 Cloudflare）。曾经导出过一个 Rust `RequestHandler` adapter，但它**从未被调用**（幻影），并已在 2026-05-30 评审后**删除**（`rust_core` 里已无 `requester/`、`Cargo.toml` 无 `reqwest`）。fetch 是 I/O 绑定，Rust 在此**无吞吐收益**。
> - **2026-06-02 查漏复扫结论：无遗漏。** 对评审未覆盖的 ~42 个大模块（存储/DB 内部、迁移/对账工具、质量评分、日志分析、爬虫 runtime、各 integrations、ops）按“CPU 密集 + 高频 + 算法稳定”判据逐一分类，**0 个真候选**——它们要么 I/O 绑定、要么一次性工具、要么热路径已被 Rust 接管。
> - **权威现状以这两份为准**：[ADR-041](../_archive/ADR-041-Rust-Fallback-Policy/ADR-041-rust-fallback-policy.md)（Rust 回退策略，已完成）与 2026-05-30 架构评审 [architecture-review-2026-05-30.html](architecture-review-2026-05-30.html)（候选卡片 1–4）。

## 执行摘要

基于性能和内存安全的角度，采用**选择性重构**策略。目前绝大多数核心计算模块（HTML 解析、代理池、数据模型、历史管理、磁链提取、CSV 操作、URL 处理、掩码函数）均已用 Rust 重写完成；**HTTP fetch 保持 Python**（`curl_cffi`，见顶部横幅）。剩余业务逻辑层保持 Python 实现。
...
| ~~HTTP 请求处理 (requester)~~ | —— | —— | ❌ **已撤销**：幻影 adapter，从未被调用，已删除；fetch 保持 Python（`curl_cffi`） |
...
- **I/O 绑定**：主要时间消耗在网络请求（Python `curl_cffi`），而非 CPU
...
```
- HTML 解析已用 Rust ✅
- 网络 I/O 为 Python `curl_cffi`（I/O 绑定，Rust 无加速收益）
- CSV/历史记录已用 Rust ✅
...
```
总耗时 100%：
├─ 网络 I/O (HTTP 请求)     60%  Python curl_cffi（I/O 绑定，Rust 无收益）
...
- **网络请求**：Python `curl_cffi`（I/O 绑定；内存安全由 Python 运行时保证）
...
| HTML 解析器 | ⭐⭐⭐ | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ | ✅ **已完成** |
| 代理池 | ⭐⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ | ✅ **已完成** |
| HTTP 请求 | —— | ⭐ | ⭐ | ❌ **保持 Python**（I/O 绑定；幻影 Rust adapter 已删） |
| 数据模型 | ⭐⭐ | ⭐⭐⭐ | ⭐⭐⭐⭐⭐ | ✅ **已完成** |
| CSV 处理 | ⭐⭐⭐⭐ | ⭐⭐ | ⭐⭐⭐ | ✅ **已完成** |
| 业务逻辑 | ⭐⭐⭐⭐⭐ | ⭐ | ⭐⭐ | ❌ **不建议** |
| API 调用脚本 | ⭐⭐⭐⭐ | ⭐ | ⭐ | ❌ **不建议** |
...
1. **当前架构**：
   - Rust 核心模块（解析、代理、历史管理、CSV、磁链提取、URL 处理、掩码）✅
...
1. **已完成**：HTML 解析（最大瓶颈）✅
2. **保持 Python**：网络请求为 I/O 绑定（`curl_cffi`），Rust 无收益
3. **已完成**：并发安全 ✅
4. **已完成**：历史记录管理 ✅
5. **已完成**：CSV 处理 ✅
6. **已完成**：磁链提取 ✅
...
3. ❌ **HTTP 请求处理**：保持 Python（`curl_cffi`）；幻影 Rust adapter 已删
4. ✅ **数据模型**：序列化优化
...
*报告生成时间：2026-02-18*  
*最后更新时间：2026-06-02（修正 HTTP requester 误述 + 补入 gap-scan 结论；见顶部横幅）*  
*基于项目当前架构和代码分析*
