...
2. **chdir/sys.path** — removed the import-time `os.chdir(REPO_ROOT)` + `sys.path.insert(...)` from all five service modules (kept `REPO_ROOT`); moved `os.chdir(REPO_ROOT)` to the first statement of each CLI adapter's `main()`. Importing a service no longer changes cwd; CLI behavior is unchanged (production runs `python -m apps.cli.*` from repo root).
...
6. **Repo layer** — notify session-start read now uses `SessionsRepo(_conn).get(_sid).created_at` over the explicit `REPORTS_DB_PATH` connection (sqlite-local preserved).
...
- (2) Importing any integration service no longer changes the process cwd / `sys.path`. Production CLI behavior is identical (cwd is already repo root; `main()` re-establishes it). This removes a cross-contamination hazard for the REST API, tests, and concurrent tasks.
