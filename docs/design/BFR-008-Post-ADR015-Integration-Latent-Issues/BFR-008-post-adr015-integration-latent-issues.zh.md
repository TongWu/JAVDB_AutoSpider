...
2. **chdir/sys.path** —— 从 5 个 service 模块移除导入期的 `os.chdir(REPO_ROOT)` + `sys.path.insert(...)`（保留 `REPO_ROOT`）。chdir 改放到每个 CLI adapter 的**模块顶部、在 integration 导入之前**。（最初放在 `main()` 里，但 PR 评审（CodeRabbit/Codex）正确指出那样太晚：导入 adapter 的 `options` 子模块会触发包 `__init__`，后者导入 service，而 service 在导入期就执行 `setup_logging()`（建 `logs/` 文件 logger）和 `cfg()` 解析——这些必须在 cwd == repo 根时运行。因此 chdir 必须先于 adapter 里第一条 `javdb.integrations.*` 导入。）净效果：导入 *service* 不再改 cwd（REST API / 测试 / 并发任务不再被污染）；CLI 入口仍会在其 service 加载前把 cwd 设为 repo 根。生产行为不变（CLI 以 `python -m apps.cli.<command>` 运行——例如 `apps.cli.notify.email`、`apps.cli.qb.uploader`——从 repo 根运行）。
...
6. **仓储层** —— notify 读 session 起始时间改为通过 `SessionsRepo(_conn).get(_sid).created_at`（走仓储层）。连接来自 `get_local_sqlite_db(REPORTS_DB_PATH)`——这个专用 helper 始终返回原生 `sqlite3.Connection`（绝不走 D1），而非按后端路由的 `get_db()`。这样既保持读取 sqlite-local（邮件模块不能掩盖 D1 滞后），又满足 `SessionsRepo` 仅支持 sqlite 连接的契约（即便 `STORAGE_BACKEND=dual`）。（CodeRabbit 建议用 `get_db('reports')`，但它在 dual 下会自动路由到 D1 连接,会同时破坏这两条不变量。）
...
- (2) 导入任何 integration **service** 不再改进程 cwd / `sys.path`。CLI **adapter** 仍会在导入期（加载其 service 之前）chdir 到 repo 根——adapter 是入口、不被 REST API 导入,故无害。生产 CLI 行为完全一致（cwd 本就是 repo 根）。这消除了对导入 service 的 REST API、测试、并发任务的交叉污染隐患。
