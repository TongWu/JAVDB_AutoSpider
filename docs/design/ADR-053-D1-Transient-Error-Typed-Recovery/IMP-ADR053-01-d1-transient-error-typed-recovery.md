...
> **Status: 🔲 Proposed (2026-06-13).** Single PR; behaviour-preserving (representation change only). Authored from [ADR-053](ADR-053-d1-transient-error-typed-recovery.md).
...
- [ ] **Step 0.1 — Green baseline:** `pytest tests/unit -k "d1 or dual or recovery or backoff or outbox" -q`.
- [ ] **Step 0.2 — Enumerate every set/read site + the ignores:**
  ```bash
...
- [ ] **Step 1.1 — Four fields on `D1TransientError`** with immutable defaults (per ADR-053 D1).
- [ ] **Step 1.2 — `class D1RecoveryBlockerError(RuntimeError)`** with a docstring, beside `D1Error`.
...
- [ ] **Step 2.1 — Blocker:** `:259-263` → `raise D1RecoveryBlockerError("unresolved D1 recovery work for ordering key {key}; drain it before flushing queued writes")`.
- [ ] **Step 2.2 — Drop the `# type: ignore[attr-defined]`** from the 6 cross-module + 2 intra-module assignments (`:292`, `:293`, `:448`, `:459`, `:483`, `:596`, `:601`, `:608`) — now declared fields.
- [ ] **Step 2.3 — Intra-module reads** (`_compute_backoff:490`, `:507`) read `exc.retry_after`/`exc.is_export_lock` directly (drop the `getattr`).
...
- [ ] **Step 3.1 — `_requires_durable_recovery`** → `isinstance(exc, D1TransientError) and exc.d1_recovery_outbox_required and not exc.d1_recovery_durable`.
- [ ] **Step 3.2 — `_blocks_queued_recovery_flush`** → `self._d1_queued_pending_writes > 0 and isinstance(exc, D1RecoveryBlockerError)`; **delete** `"unresolved D1 recovery work" in str(exc)`.
- [ ] **Step 3.3 — Imports:** add `D1TransientError`, `D1RecoveryBlockerError` to the `d1_client` import.
...
- [ ] **Step 4.2 — Backoff:** `retry_after`/`is_export_lock` still drive `_compute_backoff` as before.
- [ ] **Step 4.3 — CONTEXT.md:** add the two terms.

  **Verification gate:** new tests green.

## Task 5: Final gates

- [ ] `pytest tests/unit -k "d1 or dual or recovery or backoff or outbox" -q` green.
- [ ] `grep -rn "type: ignore\[attr-defined\]" javdb/storage/d1_port.py javdb/storage/dual_connection.py` → none for the recovery signals.
- [ ] `grep -rn "unresolved D1 recovery work" javdb/storage` → only the `D1RecoveryBlockerError` message (no `str(exc)` match).
- [ ] `ruff check javdb/storage/d1_client.py javdb/storage/d1_port.py javdb/storage/dual_connection.py` clean.
- [ ] `git diff --stat`: typed fields + subclass added; 8 ignores removed; string fallback deleted.
