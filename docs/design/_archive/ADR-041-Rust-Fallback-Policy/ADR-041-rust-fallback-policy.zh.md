...
- 2026-06-02：**已完成并归档。** IMP-ADR041-01 全部交付物已在 `main` 核实落地(parity 套件已删;新增 `tests/unit/test_fallback_shape.py`;移除 Python `ProxyPool`/`ProxyBanManager` 主体并守卫两个工厂;magnet 回退 `debug`→`warning`;CONTEXT.md 术语 + ADR-020 反向链接均已落地)—— 经 PR #131 合入。状态由 Proposed 改为 Completed;ADR 文件夹移入 `_archive/`。推迟的幽灵 HTTP requester(评审卡片 2)已独立解决:删除了那个 dead Rust adapter —— fetch 保持 Python(`curl_cffi`)。
- 2026-06-13：Rust-Required 层由 [ADR-048](../../ADR-048-Rclone-Module-Split/ADR-048-rclone-module-split-and-folder-dedup-rust.zh.md) 扩展 —— rclone 文件夹去重级联加入 ProxyPool/ProxyBanManager 成为 Rust-Required 模块,触发轴为新增的不可逆/blast-radius(ADR-048 D6)。
