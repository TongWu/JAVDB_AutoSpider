...
**状态**：已完成 —— 2026-05-30 确认 Production gated bake 通过；代码默认值对 self-hoster 继续保持 opt-in，Production 环境继续启用全部 ADR-010 gates
**日期**：2026-05-19
**决策者**：D1 统一读写端口 brainstorming + grill 会话
**前置**：[ADR-006](../ADR-006-Pending-Mode-Rollout/ADR-006-pending-mode-default-rollout.md) 已将 Pending Mode 作为默认写入路径；[ADR-009](../ADR-009-D1-Drift-Classifier/ADR-009-d1-drift-classifier-and-diagnose.md) 记录了近期 D1 瞬时失败和 drift 响应。
**关联实现计划 (Related Implementation Plans)**：[IMP-ADR010-01](IMP-ADR010-01-d1-access-port-phase1-core.md)（Phase 1 — 端口核心）、[IMP-ADR010-02](IMP-ADR010-02-d1-access-port-phase2-recovery-outbox.md)（Phase 2 — 恢复 outbox）、[IMP-ADR010-03](IMP-ADR010-03-d1-access-port-phase3-safe-batching.md)（Phase 3 — 安全批处理）、[IMP-ADR010-04](IMP-ADR010-04-d1-access-port-phase4-startup-replay.md)（Phase 4 — 启动重放）、[IMP-ADR010-05](IMP-ADR010-05-finish-recovery-facade.md)（Phase 5 — 收尾恢复 facade）
...
2026-05-30 closeout：Production 环境已在 2026-05-26 13:51 UTC 启用 `D1_RECOVERY_OUTBOX_ENABLED`、`D1_BATCHING_ENABLED` 和 `D1_STARTUP_REPLAY_ENABLED`。gate 翻转后，DailyIngestion 在 2026-05-27、2026-05-28、2026-05-29 均成功；QBFileFilter 在 2026-05-26 至 2026-05-29 均成功；StaleSessionCleanup 在 2026-05-27 至 2026-05-30 均成功。最新 `reports/D1/d1_port_summary.json` 显示 `outbox_queued=0`、`outbox_dead_lettered=0`、`permanent_errors=0`、`transient_errors=0`；`reports/D1/` 下不存在 recovery outbox / dead-letter 文件；2026-05-27 至 2026-05-29 的 pending verification 记录均为 `pending_residual_count=0` 且 `derived_recompute_drift=0`。

Closeout 决策：代码默认值继续保持保守，对 self-hoster 仍是 opt-in；Production 环境 gate 推广视为完成。未来若要翻转代码默认值，应作为新的显式变更处理，因为那会改变 self-hoster 行为，而不是完成本 ADR。
...
| Phase 5 | [IMP-ADR010-05](IMP-ADR010-05-finish-recovery-facade.md) | 恢复 helper 改为 duck-type 连接 facade 方法；commit 路径移除 `current_backend()` 读取 | —（重构；无 rollout gate） |

默认值晋级 gate 已使用 `d1_port_summary.json`、pending verify 记录以及 drift/dead-letter 缺失情况完成判断。2026-05-30 closeout 未发现新的 pending residual、dead letter 或无法解释的 drift。代码默认值继续对 self-hoster 保持 opt-in；Production 通过 GitHub Environment variables 保持 gates 启用。
...
- [ADR-005](../ADR-005-Db-Py-Retirement/ADR-005-db-py-retirement-and-repo-pattern.md) —— Repo 迁移与最终 `db.py` 退役。
- [ADR-006](../ADR-006-Pending-Mode-Rollout/ADR-006-pending-mode-default-rollout.md) —— Pending Mode 默认和 bake-gate 模型。
