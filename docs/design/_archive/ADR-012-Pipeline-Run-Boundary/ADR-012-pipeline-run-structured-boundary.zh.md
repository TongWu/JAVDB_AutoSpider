...
**状态**：已完成 - Phase 1 已完成（2026-05-25 sign-off），Phase 2 已完成并通过 review hardening（2026-05-25 已验证），最终阶段已完成并通过 review hardening（2026-05-26 已验证）
**日期**：2026-05-20
...
- Phase 2 - Pipeline 通过 in-process runner 调 Spider，同时 qB uploader、PikPak、email 和 rclone dedup 继续保持 subprocess steps。已通过 [IMP-ADR012-03](IMP-ADR012-03-pipeline-run-phase2-in-process-spider.md) 完成、review hardening 并验证。
- Final phase - GitHub workflows 和轻量 API task metadata 改用 result JSON，不再 grep stdout。已通过 [IMP-ADR012-04](IMP-ADR012-04-pipeline-run-final-result-consumption.md) 完成、review hardening 并验证。
