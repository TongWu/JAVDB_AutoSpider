...
> **Status: ✅ Completed (2026-06-14).** Single PR; pure relocation, no behaviour change. Authored from [ADR-050](ADR-050-pending-verify-record-builder.md).
...
**Tech Stack:** Python 3, `pytest`, JSONL (`reports/D1/d1_drift.jsonl`), `drift_io.append_jsonl_record`.
...
- [x] **Step 0.1 — Green baseline:** focused commit/rollback/pending tests recorded before relocation.
- [x] **Step 0.2 — Capture the three records.** Run a fixture session through each path (commit success, commit failure, rollback) and save the emitted JSONL lines as the equivalence golden. Enumerate the exact field set each source emits (confirm the 17-field core + each source's extras).
...
- [x] **Step 1.1 — `pending_verify.py`:** define every `F_*` field-name constant and `build_pending_verify_record(...)` assembling the core + `source`-conditional extras + merged `rollback_extras`, `shadow_audit_result`. Pure — no imports of `HistoryRepo`/`get_db`.
- [x] **Step 1.2 — `test_pending_verify_builder.py`:** assert the builder reproduces each Task 0.2 golden when given that source's inputs.
...
- [x] **Step 2.1 — `commit_session.py`:** delete `_emit_pending_verify`; the 4 call sites delegate + keep `append_jsonl_record` and `write_github_output`; shadow-audit computed at the call site and passed as `shadow_audit_result`.
- [x] **Step 2.2 — `sessions/commit.py`:** delete `_emit_commit_metrics`; the call site delegates with `stats` pre-fetched (try/except → `{}`), `shadow_audit_result=None`.
- [x] **Step 2.3 — `rollback/core.py`:** delete `_emit_pending_verify_for_session`; the 3 call sites delegate with `rollback_extras`; move the `pre_write_mode != pending` guard out to the call sites.
...
- [x] **Step 3.1 — `log_analysis.py`:** `_CRITICAL_ALERT_FIELDS` and `_evaluate_pending_alerts` import and use the `F_*` constants.
- [x] **Step 3.2 — `test_commit_session_events.py`:** re-target the monkeypatch.
- [x] **Step 3.3 — CONTEXT.md:** add the two terms.
...
- [x] Focused pending/session gate green (`pytest tests/unit/test_pending_verify_builder.py tests/unit/test_pending_alert_decision.py tests/unit/test_commit_session_events.py tests/unit/test_email_notification_p0.py tests/unit/test_rollback_pending_mode.py tests/unit/test_session_helpers.py tests/unit/test_system_state_repo.py ... -q`, `203 passed` in the combined ADR-050/052 gate).
- [x] `grep -rn "_emit_pending_verify\b\|_emit_commit_metrics\b\|_emit_pending_verify_for_session\b" javdb apps` → empty (all three deleted).
- [x] JSONL equivalence: builder tests cover the three source shapes key-for-key.
- [x] `ruff check` on the touched pending/session/rollback/log-analysis paths clean.
