...
- **PR-5** ✅ shipped (2026-05-22): deleted `javdb/storage/db/db.py`; the ADR-001 modules remain as the canonical implementation modules, not shell facades. (One deferred caller-migration item — the inventory-alignment migration tool's history-write path — was left stubbed by PR-4/PR-5 and only completed on 2026-06-15; see amendment 9.)
...
- **2026-05-17 amendment 1**: After this ADR was accepted, [ADR-007](../ADR-007-Monorepo-Restructure/ADR-007-monorepo-restructure-2026-05.md) reorganised the Python namespace (`packages/python/javdb_*` → top-level `javdb/`). Any PRs from this ADR's implementation order that have not yet merged when ADR-007 Phase 1 lands must operate on the new paths:
...
- **2026-06-15 amendment 9**: **Deferred PR-4/PR-5 item closed — the inventory-alignment migration tool was rewritten to staging+commit.** When PR-4 removed `db_upsert_history`, `javdb/migrations/tools/align_inventory_with_moviehistory.py` still imported it. PR-4's verification step flagged the leftover hit as "known PR-5 scope" ([IMP-ADR005-01](IMP-ADR005-01-drop-audit-mode.md)), but PR-5 ([IMP-ADR005-02](IMP-ADR005-02-delete-db-facade.md)) only rewrote the tool's **imports** — the write-path rewrite the flag called for was never executed. The tool was left with an `_audit_retired_stub` plus a fail-fast guard that aborted any non-dry-run run. Because nothing tracked the deferral, it went unnoticed until the WeeklyDedup → Migration `align_inventory_history` job (which runs `dry_run=false`) failed every week with `db_upsert_history was removed by ADR-005 PR-4`. The tool now opens its own pending `ReportSessions` row, stages each aligned movie/torrent through `HistoryRepo.stage_movie` / `stage_torrent`, and commits the session at end of run (rolling it back on failure) — the same staging+commit path the spider uses. The stub and guard are gone and `--dry-run` is no longer required to run it. Root cause and fix are recorded in [BFR-019](../../BFR-019-Align-Tool-Stuck-Dry-Run/BFR-019-align-tool-stuck-dry-run.md).
