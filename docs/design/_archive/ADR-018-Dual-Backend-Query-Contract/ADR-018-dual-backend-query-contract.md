...
| Field       | Value                                                                 |
| ----------- | --------------------------------------------------------------------- |
| **Status**  | Completed — Phases 1 & 2 implemented 2026-05-30; local verification complete; optional Phase 3 ("eliminate", D7) deferred until recurring drift justifies it |
| **Date**    | 2026-05-29                                                            |
| **Authors** | Ted                                                                   |
| **Related** | [ADR-029](../ADR-029-Web-Security-Hardening/ADR-029-web-security-hardening.md) (auth hardening — owns token revocation), [ADR-017](../ADR-017-Cloudflare-First-Deployment/ADR-017-cloudflare-first-deployment.md) (dual-backend split), [ADR-010](../ADR-010-D1-Access-Port/ADR-010-d1-access-port.md) (D1 access port) |
...
| Phase 2 | [IMP-ADR018-02](IMP-ADR018-02-ts-consume-and-dispatch.md) | Vendored golden in TS repo, CI freshness-diff vs Python `main` + vitest conformance for `buildMovieQuery` etc., **SQL-backed overlapping `stats` trend builders** (`success_rate`, `movies`, `torrents`, `history_growth`, `pikpak`, `dedup`), `repository_dispatch` re-vendor automation (D6) | `duration` (Python returns 501), `proxy_bans` (log-derived), and TS-only ADR-027 stats metrics remain outside ADR-018 |
...
- 2026-05-30: Phase 2 implemented ([IMP-ADR018-02](IMP-ADR018-02-ts-consume-and-dispatch.md)), Tasks 1–7. TS pure builders extracted (`buildMovieWhere`/`buildTorrentWhere` in `server/routes/history.ts`, `buildSessionQuery` in `server/routes/sessions.ts`); golden vendored into `server/__tests__/fixtures/` via `scripts/fetch-query-golden.mjs` (`gen:query-golden`); vitest conformance (`server/__tests__/query-contract.test.ts`) pins all 25 golden cases. `ci.yml` gains the golden freshness gen-diff **and a `test:server` step** — CI did not previously run `test:server`, so the conformance guard would otherwise not gate CI. Cross-repo automation: Python `publish-query-contract.yml` (regenerate→commit→`repository_dispatch`) + TS `revendor-query-golden.yml` (listen→re-vendor→PR). **Movie/torrent builders matched directly (no drift); the one pre-existing drift — the TS `/sessions` handler binding `limit` instead of Python's `limit + 1` — was reconciled toward Python (over-fetch lookahead; fixes a phantom `next_cursor`), pinned by two new route tests.** Both drift simulations (TS-side clause change → conformance red; Python-side golden change → freshness red) confirmed the guard. **`stats` (Task 8) stays deferred.** Pending ops: create `WEB_REPO_DISPATCH_TOKEN` in the `Production` environment + post-merge CI/dispatch dry-runs.
- 2026-05-30: Phase 2 Task 8 completed. Python extracted importable SQL-backed `/api/stats/trend` builders from `apps/api/routers/stats.py` into `apps/api/routers/stats_query_builders.py`; the golden now includes 31 cases with 6 `stats_trend_query` cases and content-hash `85fef51b76f3decc`. TS added matching `buildStatsTrendQuery`, conformance coverage, and a re-vendored fixture. Scope is intentionally limited to overlapping SQL-backed metrics (`success_rate`, `movies`, `torrents`, `history_growth`, `pikpak`, `dedup`); `duration`, `proxy_bans`, and TS-only ADR-027 metrics remain out of contract. Local Task-9 verification is complete (`test:server`, `test:unit`, `typecheck`, `build`, and both drift simulations). The remaining remote freshness/dispatch gates are post-merge because the TS CI and re-vendor workflow both fetch Python `main`.
- 2026-06-04: ADR-047 extended this guard narrowly after a follow-up drift scan found bugs in ADR-018's deliberately-unguarded surface. The SQL golden now also includes `movie_count` / `torrent_count` capped history count statements (53 cases total, content-hash `439006fcad451495`), with matching TS conformance. Static `/summary` queries and the `write_mode` NULL default are guarded by symmetric per-backend unit tests, not by broad static-query pinning; ADR-018 D3 and D7 remain unchanged.
- 2026-06-15: D7 ("eliminate") was executed for the static surface by [ADR-055](../../ADR-055-Dual-Backend-Contract-Single-Source/ADR-055-dual-backend-contract-single-source.md); this extends, but does not supersede, this ADR's guard for dynamic builders.
