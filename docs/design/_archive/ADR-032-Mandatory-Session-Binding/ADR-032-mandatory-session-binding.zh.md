...
| 字段       | 值                                                                    |
| ---------- | --------------------------------------------------------------------- |
| **状态**   | Completed — Phase 1 & 2 已实现 2026-05-29；Phase 3 延后（D4） |
| **日期**   | 2026-05-29                                                            |
| **作者**   | Ted                                                                   |
| **关联**   | [ADR-005](../ADR-005-Db-Py-Retirement/ADR-005-db-py-retirement-and-repo-pattern.md)（本 ADR 补完 ADR-005 amendment-2 的目标；见 ADR-005 amendment-8） |
...
- 2026-05-29：Phase 2 已实现（[IMP-ADR032-02](IMP-ADR032-02-single-repo-interface.md)，拆 2a/2b）。**2a**（PR #127）：补齐缺失的薄 Repo 方法（`SessionLifecycleRepo`/`HistoryRepo`），并把全部外部 `db_*` 调用方（integrations、apps/cli、spider、workflow、infra）迁到 Repo。**2b**：把实现层（Repos + `sessions/` + `rollback/`）的 `db_*` 导入从 `javdb.storage.db` facade repoint 到 `_db_*` 子模块，并新增基于 ast 的**边界测试**，禁止非 storage/非 migration 的生产代码经 facade 导入 `db_*`。范围校正：硬删导出（原 Task-6 de-export 证明）实测约 46 文件 / 33 测试，相对 SQLite 退役方向价值边际低，故 Phase 2b **用测试而非删除来强制边界**——`__all__` 保留。Phase 3（删 `set/get_active_session_id` 全局读取者）仍延后（D4）。
