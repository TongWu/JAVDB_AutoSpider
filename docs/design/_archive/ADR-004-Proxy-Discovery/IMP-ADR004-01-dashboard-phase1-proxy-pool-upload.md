# IMP-ADR004-01: Dashboard Overhaul — Phase 1: Runner Reports `proxy_pool` to Worker
...
**Related**: implements [ADR-004](ADR-004-proxy-discovery-via-runner-pool-upload.md); prerequisite for [IMP-ADR003-01](../ADR-003-Metrics-Pipeline/IMP-ADR003-01-dashboard-phase2-worker-backend.md)
...
## Task 5: Documentation — update CLAUDE.md and docs/design

**Files:**
- Modify: `CLAUDE.md` (env vars section if applicable, or imports section)
- Modify: `docs/handbook/en/developer/` (if there's a runner registry guide)

- [ ] **Step 1: Search for existing runner-registry documentation**

Run: `grep -rn "proxy_pool_hash\|RunnerRegistryClient\|runner_registry" CLAUDE.md docs/design/ docs/handbook/en/ 2>/dev/null | head -20`

- [ ] **Step 2: Find the right doc to update**

If `grep` finds a `docs/handbook/en/developer/*.md` file describing the runner registry payload, add a paragraph there. Otherwise, the only doc that needs an update is the CONTEXT.md entry for `RunnerRegistry DO` (which already mentions Phase 1 in the `proxies_seen` description — confirm this is accurate).
