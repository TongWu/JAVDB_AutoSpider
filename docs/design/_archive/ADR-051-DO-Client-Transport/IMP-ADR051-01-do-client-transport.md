...
> **Status: ✅ Completed (2026-06-14).** Single PR; no public-API change. Authored from [ADR-051](ADR-051-do-client-transport-consolidation.md).
...
- [x] **Step 0.1 — Green baseline:** `pytest tests/unit/test_proxy_coordinator_client.py -q`.
- [x] **Step 0.2 — Enumerate every async-queue producer and the sentinel:**
  ```bash
...
- [x] **Step 1.1 — Add `AsyncReportEvent`** frozen dataclass + module-level `ASYNC_QUEUE_SENTINEL`.
- [x] **Step 1.2 — `report_async()`** enqueues `AsyncReportEvent(...)` (signature unchanged); annotate the queue `Queue[AsyncReportEvent]`.
- [x] **Step 1.3 — `_async_report_loop`:** `if item is ASYNC_QUEUE_SENTINEL: break`; access `item.proxy_id`/`item.kind`/… by name; **delete** the `len(item) > 2/3/4` branches.
...
- [x] **Step 2.1 — `lease()`:** `data = self._do_request('POST', '/lease', body)`; then the existing health-cache write, `ProxyHealthSnapshot`, `banned_until`/`cf_bypass_until` parsing, `LeaseResult` — on `data`. Remove the inline `try/except` transport block.
- [x] **Step 2.2 — `report()`:** `data = self._do_request('POST', '/report', body)`; then `penalty_factor`/`recent_event_count` extraction, `ReportResult`. Remove the inline transport block.
- [x] **Step 2.3 — Remove orphans** the routing creates (unused imports, now-dead local helpers).
...
- [x] **Step 3.1 — Dict-guard tests:** feed a bare list / non-dict JSON to `lease()` and `report()`; assert `CoordinatorUnavailable` (covers the guard `_do_request` adds that the old inline code lacked).
- [x] **Step 3.2 — Sentinel/event tests:** the loop exits on `ASYNC_QUEUE_SENTINEL`; a real `AsyncReportEvent` is dispatched by field.
...
- [x] `pytest tests/unit/test_proxy_coordinator_client.py tests/unit -k "coordinator or proxy_client" -q` green.
- [x] `grep -n "len(item) >" javdb/proxy/coordinator/proxy_coordinator_client.py` → empty (dead compat gone).
- [x] No public-API diff: `report_async`/`lease`/`report`/`LeaseResult`/`ReportResult` signatures unchanged; the 4 external `report_async` call sites untouched.
- [x] `ruff check javdb/proxy/coordinator/proxy_coordinator_client.py` clean.
