...
**状态**: 已完成 —— PR-A/C/D/E 已合入；**PR-F sign-off completed on 2026-05-21**（operator-approved 7-day clean bake bypass）
**日期**: 2026-05-16
**决策者**: 架构深化第二轮（[ADR-005](../ADR-005-Db-Py-Retirement/ADR-005-db-py-retirement-and-repo-pattern.md) 的前置依赖）
**后继触发**: ADR-006 已完成；ADR-005 PR-2 已解锁（PR-1 已交付）。该 bypass 不授权 ADR-005 PR-4 提前删除 audit 表；必须先通过 D10 三项核查。

## 待办 (Outstanding Work)

- **PR-F（sign-off）✅** —— 2026-05-21 由维护者基于连续一周无 pending-mode 问题批准 bypass 30 天剩余期限，并在 ADR-005 顶部追加 "ADR-006 sign-off completed on 2026-05-21"。
- `BakeCheck.yml`（`cron: 0 4 * * *`、`since: 2026-05-16`）保留为后续回归监控；ADR-005 PR-4 前仍需重新运行 D10 三项核查。
...
- **2026-05-17 amendment 2**：ADR-006 接受后，[ADR-007](../ADR-007-Monorepo-Restructure/ADR-007-monorepo-restructure-2026-05.md) 对 Python namespace 做了重组（`packages/python/javdb_*` → 顶层 `javdb/`）。本 ADR 实施顺序里**尚未合并的 PR**，在 ADR-007 Phase 1 落地后必须按新路径操作：
...
- **2026-05-21 amendment 4**：**PR-F sign-off 采用 operator-approved 7-day clean bake bypass。** 原 D4 的 30 天 bake 期限由维护者显式 bypass：当前已连续一周无 pending-mode 问题，允许继续 ADR-005 PR-2。该 bypass 只解除 PR-2 起始阻塞，不提前授权 PR-4 的 audit 表删除；PR-4 前仍需 D10 三项核查通过。
...
修改：
- `javdb/storage/db/db_session.py:188` `return "audit"` → `return "pending"`（路径按 amendment 2 更新）
...
D1-D3 部署后，**bake 至少 30 天**，除非 operator 明确批准更短的 clean bake。期间运维监控：
...
- bake 结束或 operator-approved bypass 后，在 ADR-005 PR-4 前重新运行 D10 三项核查；三项全部通过仍是删除 audit 表的前置条件。

### D5：bake 期间禁止 ADR-005 任何 PR

> **见上方 amendment 3 和 amendment 4。** 此决策先被收窄为 PR-1 bake-safe 豁免，随后 2026-05-21 的 PR-F sign-off 采用 operator-approved 7-day clean bake bypass 解锁 PR-2。PR-4 的 audit 表删除仍必须先通过 D10 三项核查。

ADR-005 的 PR-1 已作为 bake-safe 增量变更交付。ADR-005 PR-2 可在 2026-05-21 sign-off bypass 后继续。ADR-005 PR-4 不属于此次 bypass 范围，仍需 D10 三项全部 sign off 后才能启动。
...
PR-F  已于 2026-05-21 通过 operator-approved 7-day clean bake bypass 完成：
      在 ADR-005 顶部插入 "ADR-006 sign-off completed on 2026-05-21" 标记，
      解锁 ADR-005 PR-2，同时保留 PR-4 前的 D10 gate
```

每个 PR 独立可回滚。PR-A / PR-C / PR-D 是核心，PR-E 已先行；PR-F 已通过 operator-approved bypass 完成。`BakeCheck.yml` 继续作为回归监控，而不是 active blocker。
...
1. **30 天 bake 已针对 ADR-005 PR-2 被 bypass**——2026-05-21 operator sign-off 以继续 BakeCheck 回归监控换取提前推进
...
- **后继**：[ADR-005](../ADR-005-Db-Py-Retirement/ADR-005-db-py-retirement-and-repo-pattern.md) — PR-2 可在 2026-05-21 sign-off bypass 后继续；PR-4 仍需 D10 三项核查通过
- **修正历史承诺**：[ADR-001](../ADR-001-Split-Db-Module/ADR-001-split-db-module.md) Phase 3 中"Pending Mode 默认"的承诺由本 ADR 真正兑现
...
- D10 核查所用 SQL：
  ...
  ```
- 已退役 auto-fallback（由 D3 告警+暂停取代）：[`.github/workflows/DailyIngestion.yml`](../../../../.github/workflows/DailyIngestion.yml)、`apps/cli/db/pending_alert.py`（路径按 amendment 2 更新）
- 默认实现：[`javdb/storage/db/db_session.py`](../../../javdb/storage/db/db_session.py)（路径按 amendment 2 更新）
