...
**Files:**
- Read-only: `docs/ai/adr/ADR-007-deletion-manifest.md` (consume; do not modify yet — that happens at the end as "tick checkboxes")
...
- `docs/en/**/*.md` (and the paired `docs/zh/**/*.md`)
- Skip: `docs/ai/architecture/audit-report-*.md` (historical record; preserve as-is)
- Skip: ADR files in `docs/ai/adr/` (they document the OLD paths in their decision history; the "Amendments" in ADR-005/006 already explain the migration)
...
find docs/en docs/zh -name "*.md" -not -path "*/audit-report-*" 2>/dev/null | xargs sed -i.bak -f /tmp/phase3-doc-rewrite.sed
sed -i.bak -f /tmp/phase3-doc-rewrite.sed README.md README_CN.md CLAUDE.md CONTEXT.md
find docs/en docs/zh -name "*.bak" -delete 2>/dev/null
...
```bash
grep -rE "packages\.python\.javdb_|from utils\.|packages/python/javdb_|packages/rust/javdb_rust_core" docs/en docs/zh README.md README_CN.md CLAUDE.md CONTEXT.md 2>/dev/null | grep -v "audit-report-"
```
...
```bash
git add docs/en docs/zh README.md README_CN.md CLAUDE.md CONTEXT.md
...
**Files:**
- Modify: `docs/ai/architecture/python-core-mapping.md` (replace with supersession redirect)
- Modify: `docs/ai/architecture/spider-module-reorg.md` (replace with supersession redirect)
- Create: `docs/ai/architecture/python-tree-2026-05.md` (the new tree map)
...
- `docs/ai/architecture/python-core-mapping.md` — Old partial-reorg mapping (SUPERSEDED)
- `docs/ai/architecture/spider-module-reorg.md` — Old spider-only reorg status (SUPERSEDED)
...
```bash
git add docs/ai/architecture/python-core-mapping.md docs/ai/architecture/spider-module-reorg.md docs/ai/architecture/python-tree-2026-05.md
...
**Files:**
- Modify: `docs/ai/adr/ADR-007-deletion-manifest.md` (change every `- [ ]` to `- [x]` at the end of Phase 3)

- [ ] **Step 1: Replace all checkboxes**

```bash
sed -i.bak 's|^- \[ \]|- [x]|g; s|^  - \[ \]|  - [x]|g' docs/ai/adr/ADR-007-deletion-manifest.md
rm -f docs/ai/adr/ADR-007-deletion-manifest.md.bak
```

- [ ] **Step 2: Verify**

```bash
grep -c "^- \[ \]" docs/ai/adr/ADR-007-deletion-manifest.md
grep -c "^- \[x\]" docs/ai/adr/ADR-007-deletion-manifest.md
```
...
Open `docs/ai/adr/ADR-007-deletion-manifest.md` and append at the end:
...
```bash
grep -rE "packages\.python\.javdb_|packages/python/javdb_|packages/rust/javdb_rust_core" docs/en docs/zh README.md README_CN.md CLAUDE.md CONTEXT.md 2>/dev/null | grep -v "audit-report-"
```
...
- Link to ADR-007
- Link to the (now fully-ticked) deletion manifest `docs/ai/adr/ADR-007-deletion-manifest.md`
