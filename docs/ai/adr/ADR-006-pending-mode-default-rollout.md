...
- 新增 SQLite migration v14（ADR-005 的 v14 此处提前用作 schema default 切换）：
  ```sql
  -- 当前 schema: WriteMode TEXT DEFAULT 'audit'
  -- 目标 schema: WriteMode TEXT DEFAULT 'pending'
  -- 因 SQLite 不支持 ALTER COLUMN DEFAULT，需走 rebuild-table 套路
  -- 复用 db.py 现有的 _rebuild_table_with_new_ddl 助手
  ```
- D1 schema 同步（运维侧脚本）
...
```
PR-A  代码默认改 pending：db_session.py:188 + 新增 tests/unit/test_default_write_mode.py
      验证现有运行不破坏（旧 audit session 仍能完整跑完 commit/rollback）

PR-B  Schema default 切换：v14 migration drops + recreates ReportSessions with
      WriteMode TEXT DEFAULT 'pending'；同步 D1 schema
...
PR-E  CONTEXT.md / CLAUDE.md / ADR-001 docstring 修正"pending is default"失实陈述
...
每个 PR 独立可回滚。PR-A → PR-D 是核心，PR-E 可并行，PR-F 是 bake 期结束的 ceremony。
...
2. **PR-B 的 schema 重建在 D1 上失败**（D1 不支持完整 ALTER）
   - **缓解**：D1 走 `CREATE TABLE NEW + INSERT SELECT + DROP OLD + RENAME` 模式；分两个 deploy
3. **`.publish-config.yml` 已有的 `pending_mode_disabled_until` 字段被外部脚本读** → 删除引发故障
