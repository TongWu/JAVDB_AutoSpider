...
13. **生成 deletion manifest**，落地为 `docs/ai/adr/ADR-007-deletion-manifest.md`，列出 Phase 3 必须移除的每一处 compat 残留，含精确文件路径与（针对 tests）精确行号。
...
6. 更新 `docs/en/**/*.md`、`docs/zh/**/*.md`、`README.md`、`README_CN.md`、`CLAUDE.md`、`CONTEXT.md`——把所有 `packages.python.javdb_*` 与 legacy 路径引用替换为新的 `javdb.*` 路径。
7. 把 `docs/ai/architecture/python-core-mapping.md` 与 `docs/ai/architecture/spider-module-reorg.md` 标记为 **superseded**（顶部加一行指向本 ADR），内容替换为指向新地图的 redirect。
8. 新建 `docs/ai/architecture/python-tree-2026-05.md`（重构后的新地图）。
9. 触发 `.github/workflows/sync-docs-to-wiki.yml`，让 wiki 从更新后的 `docs/en/` 重新生成。
...
- `grep -rE "from (utils|api|migration|legacy)\." . --include='*.py' | grep -v __pycache__` 返回空（排除 `docs/ai/architecture/audit-report-*.md` 这种历史记录）。
...
Phase 3 必须移除的每一处 compat 残留，以行级精度落地为 `docs/ai/adr/ADR-007-deletion-manifest.md`，由 Phase 1 PR 生成并提交。Phase 3 PR 描述里引用该文件并逐项打勾。Manifest 列出：
