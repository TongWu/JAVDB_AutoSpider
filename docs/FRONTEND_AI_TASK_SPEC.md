...
| 爬虫业务逻辑 | **Python** | `scripts/spider.py` | Phase1/Phase2 筛选、翻页、限速控制、CSV 生成（~2700 行） |
...
> Python 端的 `utils/proxy_pool.py`、`utils/history_manager.py`、`api/parsers/__init__.py` 均为薄包装——优先 `import javdb_rust_core`，若不可用则回退到纯 Python 实现。
...
| 方法 | 路由 | 认证 | 请求体 | 响应 | 说明 |
|------|------|------|--------|------|------|
| GET | `/api/health` | 无 | — | `{"status":"ok","rust_core_available":true}` | 存活探针 + Rust 状态 |
| POST | `/api/parse/index` | 无 | `HtmlPayload` | `IndexPageResult` dict | 解析首页/列表页 |
| POST | `/api/parse/detail` | 无 | `HtmlPayload` | `MovieDetail` dict | 解析影片详情页 |
| POST | `/api/parse/category` | 无 | `HtmlPayload` | `CategoryPageResult` dict | 解析分类页 |
| POST | `/api/parse/top` | 无 | `HtmlPayload` | `TopPageResult` dict | 解析排行榜页 |
| POST | `/api/parse/tags` | 无 | `HtmlPayload` | `TagPageResult` dict | 解析标签筛选页 |
| POST | `/api/detect-page-type` | 无 | `HtmlPayload` | `{"page_type":"index"}` | 检测页面类型 |
...
| 公开端点（免认证） | 仅 `GET /api/health` 和 `POST /api/auth/login` 免认证。所有 `/api/parse/*` 端点建议也要求认证（防止外部滥用解析服务） | — |
...
| 校验对象 | 规则 | 涉及端点 |
|----------|------|----------|
| **Adhoc URL** | 必须为合法 URL；**host 白名单**仅允许 `javdb.com`（与 `scripts/spider.py:1442` 的 `'javdb.com' not in url` 检查对齐）；禁止 `file://`、`javascript:`、内网 IP（`127.0.0.1`、`10.*`、`192.168.*`） | `POST /api/tasks/adhoc` |
...
- 日志格式与现有 `utils/logging_config.py` 一致，使用 Python `logging` 模块。
