...
- **Rollback library layering inversion.** Plan A Task 11 (commit `9f5e66a6`) intended to move `apps/cli/rollback.py` core logic into `packages/python/javdb_platform/rollback/core.py`. The implementer kept the pipeline in the CLI module and made the library a thin adapter that delegates back into the CLI, in order to preserve ~10 existing tests that monkeypatch CLI module attributes. Result: `packages/python/javdb_platform/rollback/core.py` imports from `apps/cli/rollback.py` — the ONLY such import in `packages/`. This violates CLAUDE.md's "Always import from canonical packages" rule but is functionally correct (Task 14 HTTP endpoint imports the public library API and works). **Recommended cleanup, Phase 2 BE work:** move the helpers (`_drive_rollback`, `_resolve_target_sessions`, `_detect_cross_day`, `_resolve_failure_reason`, `_emit_metrics`, `_emit_pending_verify_for_session`) into `rollback/core.py`, rewrite test monkeypatches to target the library, and confirm `grep -rn "from apps\." packages/python/` returns zero results.
- **Commit endpoint side-effect parity.** Plan A Task 15 (commit `eb48230e`) extracted commit logic into `packages/python/javdb_platform/sessions/commit.py` for the new `POST /api/sessions/{id}/commit` endpoint, but the library does NOT replicate `apps/cli/commit_session.py`'s post-DB side-effects: coordinator `fanout_movie_claim(operation="commit")`, JSONL drift records, and `write_github_output`. The HTTP endpoint is currently a "DB-only" commit — coordinator MovieClaim stages remain `staged`. **Recommended cleanup, Phase 2 BE work:** either add `fanout_coordinator: bool = False` to `CommitRequest` and call the coordinator client, or split the CLI commit into a library that handles all side-effects.
