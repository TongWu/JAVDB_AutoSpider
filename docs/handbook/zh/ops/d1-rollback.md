...
`ReportSessions.Id` **不再**由各后端的 AUTOINCREMENT 计数器分配。应用层通过 [`generate_session_id()`](../../../../javdb/storage/db/db_session.py) 自行生成 id：
...
`ReportSessions.WriteMode`（2026-05-09 新增）选择 cleanup-on-failure 和过期 session 定时任务的调度路径。默认是 **`pending`**，ADR-005 已退役遗留 audit 路径。`JAVDB_HISTORY_WRITE_MODE=audit` 会作为遗留请求降级为 pending。参见[ADR-006](../../../design/adr/ADR-006-pending-mode-default-rollout.md)了解设计原理。
...
**为什么用暂停而非回退？** 详见 [ADR-006](../../../design/adr/ADR-006-pending-mode-default-rollout.md) §D3：旧的 audit 自动回退把 Pending Mode 故障静默降级到一个"看起来工作但不对劲"的状态，移除了修复根因的压力。暂停强迫操作员看到事故并显式介入，事件永远可见。
...
- 核心辅助函数：[`javdb/storage/db/__init__.py`](../../../../javdb/storage/db/__init__.py)、[`db_history_write.py`](../../../../javdb/storage/db/db_history_write.py)、[`db_rollback.py`](../../../../javdb/storage/db/db_rollback.py)、[`db_reports.py`](../../../../javdb/storage/db/db_reports.py)、[`db_session.py`](../../../../javdb/storage/db/db_session.py)
...
> **状态** — 已由 [ADR-005](../../../design/adr/ADR-005-db-py-retirement-and-repo-pattern.md) PR-4/PR-5 于 2026-05-22 退役。`JAVDB_HISTORY_WRITE_MODE=audit` 会降级为 pending，audit replay path 已删除，`MovieHistoryAudit` / `TorrentHistoryAudit` 不再属于当前 schema。
