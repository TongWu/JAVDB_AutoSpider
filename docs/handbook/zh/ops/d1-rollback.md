...
| StaleSessionCleanup 报告 session 为 `needs_manual_review` | 超过 `--max-age-hours` 的 `finalizing` pending session。定时任务**刻意不**恢复这类 session：恢复会把一份 ≥48 小时前的 payload 重放到正式表上，可能覆盖更晚的 run 已重爬的行 | 先检查冲突（见上方 `pending_residual_count > 0` 那一行），再逐个手动恢复：`python3 -m apps.cli.db.commit_session --session-id <id> --shadow-audit --log-level DEBUG`。不要图省事去 dispatch StaleSessionCleanup 并勾选 `apply` —— 它没有 session id 输入，会一次性恢复所有符合条件的过期 session（只有定时任务会传 `--no-resume-finalizing`），把你没做过冲突检查的那些一并重放。如果 3 次尝试仍失败，用 `python3 -m apps.cli.db.rollback --session-id <id> --no-auto-resume-finalizing --apply` 标记为 `failed`。 |
| `worker_stage_rollback_failed > 0` | Rollback CLI 无法连接到 MovieClaim coordinator | 检查 coordinator 健康状态；孤立清扫定时任务将在 4 小时内对账。 |
| 已提交 session 上 `pending_residual_count > 0` | 无法确认已应用的写入 —— **只能 drain，绝不能删** | ⚠ 绝对不要清除这些行。手动 `DELETE` 和对 `committed` session 调用 `db_commit_session_history` 走的都是 committed 分支，会删除 `ApplyState IN ('pending','applied')` 的行而**不做任何应用** —— 如果这些写入确实没落地，删除即不可恢复。正确做法是恢复：`UPDATE ReportSessions SET Status='finalizing', CommittedAt=NULL WHERE Id=? AND Status='committed'`，然后执行 `python3 -m apps.cli.db.commit_session --session-id <id> --no-claim-commit`。**每次只用 `--session-id` 恢复一个 session** —— 不要改用手动 dispatch StaleSessionCleanup：该 workflow 没有 session id 输入，勾选 `apply` 会恢复**所有**符合条件的过期 session，把你没检查过的那些一并重放。**`pending_residual_count` 不能告诉你的两件事。**（1）它不能证明写入从未落地：`_commit_session_bulk` 先在一个批次里 UPSERT 正式表，之后才在另一个批次里标记 `ApplyState='applied'`，两者之间失败就会留下「数据已应用、行仍为 `'pending'`」的状态。`'pending'` 的含义是**未确认**，这正是不能删除的原因 —— 但 drain 是一次**重放**，对已应用的行会用旧 payload 重新 UPSERT，覆盖 `SessionId`、`DateTimeVisited`、演员字段以及种子的 `MagnetUri`/`Size`/`FileCount`/`ResolutionType`。（2）它只统计 `ApplyState='pending'`，而 drain 重放的范围更大 —— `_pending_distinct_hrefs` 从**两张** pending 表里取 `ApplyState IN ('pending','applied')`，因此 `applied` 残留也会被重新 UPSERT，drain 输出里的 `pending_marked_applied` 对应的是这个更大的集合，而不是 `pending_residual_count`。**drain 前的每一项检查都必须与 drain 自身的范围一致。** 对于隔了一两个 run 以上的 session，drain 前先查 drain 将要触及的行里是否有已被更晚 session 接管的：`SELECT COUNT(*) FROM (SELECT Href FROM PendingMovieHistoryWrites WHERE SessionId=? AND ApplyState IN ('pending','applied') UNION SELECT Href FROM PendingTorrentHistoryWrites WHERE SessionId=? AND ApplyState IN ('pending','applied')) p JOIN MovieHistory m ON m.Href=p.Href WHERE m.SessionId<>?`（三个参数都填同一个 session id）。把范围收窄到单张表、或只查 `ApplyState='pending'`，都会返回假零值，直接把你送进那次会覆盖数据的 drain。不过正式表一侧只 JOIN `MovieHistory` **是**足够的：`TorrentHistory` 没有 `Href` 列 —— 它挂在 `MovieHistoryId … REFERENCES MovieHistory(Id)` 上 —— 而 drain 对它触及的每个 `Href`（含只有种子的）都会无条件写 `MovieHistory.SessionId`，因此更晚的 session 无法在不改动该列的情况下接管种子。非零意味着重放会把这些行回退到本 session 的旧 payload —— 只有在可接受的前提下才继续 drain（重爬即可恢复），否则改为直接重爬受影响的 `Href`。**不要**用「pending 的 `Href` 是否存在于 `MovieHistory`」来做分类：该表按 `Href` 全局去重，重爬的影片本来就在表里，这个检查会给出「全部已应用」的假阴性。 |
