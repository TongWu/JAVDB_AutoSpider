...
| Session reported `needs_manual_review` by StaleSessionCleanup | A `finalizing` pending session older than `--max-age-hours`. The cron deliberately does **not** resume these: the resume replays a ≥48h-old payload over the live tables and can overwrite rows a later run has since re-scraped | Check for conflicts first (see the `pending_residual_count > 0` row above), then resume by hand, one session at a time: `python3 -m apps.cli.db.commit_session --session-id <id> --shadow-audit --log-level DEBUG`. Resist dispatching StaleSessionCleanup with `apply` ticked as a shortcut — it takes no session id, so it resumes every eligible stale session at once (only the scheduled run passes `--no-resume-finalizing`), replaying any you have not conflict-checked. If 3 attempts still fail, `python3 -m apps.cli.db.rollback --session-id <id> --no-auto-resume-finalizing --apply` to mark `failed`. |
...
| `pending_residual_count > 0` on a `committed` session | Writes not confirmed applied — **drain, never delete** | ⚠ Never clear these rows. Both a manual `DELETE` and `db_commit_session_history` on a `committed` session take the committed-session branch, which deletes `ApplyState IN ('pending','applied')` **without applying anything** — unrecoverable if the writes never landed. Recover instead: `UPDATE ReportSessions SET Status='finalizing', CommittedAt=NULL WHERE Id=? AND Status='committed'`, then `python3 -m apps.cli.db.commit_session --session-id <id> --no-claim-commit`. Resume **one session at a time with `--session-id`** — do not reach for a manual StaleSessionCleanup dispatch instead: that workflow takes no session id, so ticking `apply` resumes *every* eligible stale session, replaying the ones you have not checked. **Two things `pending_residual_count` does *not* tell you.** (1) It is not proof the writes never landed: `_commit_session_bulk` UPSERTs the live tables in one batch and only marks `ApplyState='applied'` in a later one, so a failure between the two leaves applied data with `'pending'` rows. `'pending'` means *unconfirmed*, which is why deleting is never right — but the drain is a **replay**, and on an already-applied row it re-UPSERTs the old payload, overwriting `SessionId`, `DateTimeVisited`, actor fields and the torrent's `MagnetUri`/`Size`/`FileCount`/`ResolutionType`. (2) It counts only `ApplyState='pending'`, while the drain replays more than that — `_pending_distinct_hrefs` selects `ApplyState IN ('pending','applied')` from **both** pending tables, so `applied` residue is re-UPSERTed too, and `pending_marked_applied` in the drain output is measured against that larger set, not against `pending_residual_count`. **Mirror the drain's own scope in every pre-drain check.** Before draining a session more than a run or two old, check whether a later session has taken over any of the rows the drain will touch: `SELECT COUNT(*) FROM (SELECT Href FROM PendingMovieHistoryWrites WHERE SessionId=? AND ApplyState IN ('pending','applied') UNION SELECT Href FROM PendingTorrentHistoryWrites WHERE SessionId=? AND ApplyState IN ('pending','applied')) p JOIN MovieHistory m ON m.Href=p.Href WHERE m.SessionId<>?` (same session id for all three parameters). Narrowing this to one table, or to `ApplyState='pending'` alone, returns a false zero and sends you straight into the clobbering drain. Joining `MovieHistory` alone *is* enough on the live side, though: `TorrentHistory` has no `Href` column — it hangs off `MovieHistoryId … REFERENCES MovieHistory(Id)` — and the drain stamps `MovieHistory.SessionId` unconditionally for every `Href` it touches, torrent-only ones included, so a later session cannot take over the torrents without moving that column. Non-zero means the replay will regress those rows to this session's older payload — drain anyway only if that is acceptable (a re-scrape restores them), otherwise recover the affected `Href`s by re-scraping instead. Do **not** classify by whether the pending `Href`s exist in `MovieHistory`: that table is keyed by `Href` globally, so re-scraped movies are already present and the check returns a false "all applied". |
