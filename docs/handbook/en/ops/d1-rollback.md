...
|---|---|---|
| `ReportMovies`, `ReportTorrents`, `ReportSessions`, `SpiderStats`, `UploaderStats`, `PikpakStats`, `RunEventSummary`, `ParseRunFieldFill`, `OpsIncidents` | Cascade-delete by session id (`SessionId` for the FK children, `session_id` for the newer per-run event/metric tables); refuse to delete `ReportSessions` rows whose `Status='committed'` | `ReportSessions.Status TEXT DEFAULT 'in_progress'`; Phase 3 added `WriteMode` and the `finalizing` value to `Status` |
...
| `PikpakHistory`, `DedupRecords`, `InventoryAlignNoExactMatch`, `AcquisitionOutcome` | Delete session-scoped rows. `DedupRecords` soft-delete/orphan updates first snapshot their pre-image into `DedupRecordsRollback_<session_id>`, so rollback restores pre-existing rows and deletes rows created by the failed session. `AcquisitionOutcome` (ADR-033) is cleared by `session_id` — a queued outcome for a rolled-back session was undone | `SessionId` on each table (`session_id` on `AcquisitionOutcome`); per-session `DedupRecordsRollback_<session_id>` backup table |
...
| `PipelineEvent`, `EmailNotificationHistory` | **NOT rolled back** — deliberately append-only. Their session id is provenance, not an ownership pointer: `PipelineEvent` (ADR-036 event spine) records the `SessionFailed` event itself, and `EmailNotificationHistory` logs emails that were really sent (an external action a rollback cannot undo). They legitimately outlive a rolled-back session and carry no FK to `ReportSessions`, so they never violate `foreign_key_check`. The coverage guard `tests/unit/test_rollback_table_coverage.py` forces every new session-tagged table into either this row or a cascade row | `PipelineEvent.session_id`, `EmailNotificationHistory.SessionId` (provenance only) |
