...
| `d1_recovery.py` | Inspect and compact the D1 recovery outbox introduced by ADR-010. Use `inspect` for read-only inspection; `compact` moves replayed/abandoned records to `d1_recovery_outbox.processed.jsonl`. |
