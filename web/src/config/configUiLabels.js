/** 配置页展示用文案（与后端 section / key 对齐，贴近 MDC-NG 设置页） */
...
export function sectionTabLabel(section) {
...
export function fieldDescription(key, meta) {
    ...
    if (meta.readonly)
        return "由配置生成器固定，仅展示。";
    if (meta.sensitive)
        return "敏感信息；留空表示不修改服务器上已保存的值。";
    if (meta.type === "json")
        return "JSON 格式；保存后将写回环境并重新生成 config.py。";
    if (meta.type === "bool")
        return "开启或关闭该项行为。";
    if (meta.type === "int" || meta.type === "float")
        return "数值；留空可能表示不提交该项（视后端校验而定）。";
    return "保存后由后端校验并写入，与 GitHub Actions / 本地生成器字段一致。";
}
/** 路径 / 文件位置类（展示输入框 + 文件夹按钮，路径需手填） */
...
export function isPathLikeField(key, type, sensitive) {
