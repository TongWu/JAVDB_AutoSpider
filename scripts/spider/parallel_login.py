...
class LoginCoordinator:
    ...
    def handle_login_required(
        ...
        from ``scripts.spider.parallel``.  All branch logic, counters, and log
        ...
        with self._lock:
            ...
            if (
                ...
                logger.warning(
                    "[%s] [%s] Login budget exhausted (%d/%d), "
                    "treating %s as normal failure",
                    worker.proxy_name, task.entry_index,
            ...
            if self.logged_in_worker_id == worker.worker_id:
                ...
                if not need_switch:
                    logger.info(
                        "[%s] [%s] Session stale for %s, attempting re-login "
                        "(stale: %d/%d, logins: %d/%d)",
                        worker.proxy_name, task.entry_index, video_code,
                        stale_count, LOGIN_MAX_FAILURES_BEFORE_PROXY_SWITCH,
                    ...
                    logger.info(
                        "[%s] [%s] Proxy reached switch threshold for %s "
            ...
            logger.warning(
                "[%s] [%s] Login required for %s but no proxy available, "
