...
from scripts.spider.parallel_login import LoginCoordinator, requeue_front
...
class EngineTask:
    ...
    required by :class:`~scripts.spider.parallel_login.LoginCoordinator`.
    """
...
class LoginRequired(Exception):
    ...
    shared :class:`~scripts.spider.parallel_login.LoginCoordinator`.
...
class WorkerContext:
    ...
    :class:`~utils.request_handler.RequestHandler`, with automatic CF bypass
...
class _EngineWorker(threading.Thread):
    ...
    :class:`~scripts.spider.parallel_login.LoginCoordinator`::
        ...
        while True:
            ...
            try:
                ...
                if data is not None:
                    ...
                    requeue_front(self.task_queue, task)
                    logger.info(
                        "[%s] [%s] Process returned None, re-queued "
                        "(%d/%d proxies)",
                        self.proxy_name, task.entry_index,
                ...
                requeue_front(self.task_queue, task)
                logger.warning(
                    "[%s] [%s] process_fn error: %s — re-queued "
                    "(%d/%d proxies)",
                    self.proxy_name, task.entry_index, exc,
                    len(task.failed_proxies), self.total_workers,
                )
...
class FetchEngine:
    ...
    :class:`~scripts.spider.parallel_login.LoginCoordinator` integration.
            ...
            if worker._should_shortcircuit_cf():
                ...
                if html:
                    ...
                    if data is not None:
                        ...
                        return data
                    logger.debug(
                        "[%s] [%s] parse failed: CF Bypass (marked)",
                        ctx.proxy_name, task.entry_index,
                    )
                return None
