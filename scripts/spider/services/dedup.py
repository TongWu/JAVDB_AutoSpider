...
from utils.infra.config_helper import use_sqlite, use_csv, cfg
from utils.domain.contracts import (
    UNCENSORED_SENSOR_PRIORITY,
    is_uncensored_category,
    get_uncensored_priority,
)
from utils.infra.logging_config import get_logger
from utils.bridges.rust_adapters.dedup_adapter import (
...
# Priority aliases — delegated to utils.domain.contracts
...
def mark_records_deleted(
    ...
    """
    _ensure_db()
    from utils.infra.db import db_mark_records_deleted
...
def cleanup_deleted_records(
    ...
    """
    _ensure_db()
    from utils.infra.db import db_cleanup_deleted_records
...
def save_dedup_csv(csv_path: str, rows: List[Dict[str, str]]) -> None:
    ...
    if use_sqlite():
        _ensure_db()
        from utils.infra.db import db_save_dedup_records
