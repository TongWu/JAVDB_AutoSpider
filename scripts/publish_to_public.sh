...
# Workflow files to enable push trigger (uncomment)
...
# Enable docker-publish-ghcr push trigger
enable_docker_push_trigger() {
    log_info "Enabling docker-publish-ghcr push trigger for public repo..."
    
    local filepath="$PROJECT_ROOT/$DOCKER_WORKFLOW"
    if [ -f "$filepath" ]; then
        log_info "Processing: $DOCKER_WORKFLOW"
        
        # Uncomment the push trigger section
        if [[ "$OSTYPE" == "darwin"* ]]; then
            # macOS sed
            sed -i '' 's/^  # push:  # DISABLED FOR PRIVATE REPO - enabled in public repo$/  push:/g' "$filepath"
            sed -i '' 's/^  #   branches:$/    branches:/g' "$filepath"
            sed -i '' 's/^  #     - main$/      - main/g' "$filepath"
            sed -i '' 's/^  #     - master$/      - master/g' "$filepath"
            sed -i '' 's/^  #   tags:$/    tags:/g' "$filepath"
            sed -i '' "s/^  #     - 'v\*\.\*\.\*'$/      - 'v*.*.*'/g" "$filepath"
            sed -i '' 's/^  #   paths:$/    paths:/g' "$filepath"
            sed -i '' "s/^  #     - '\*\*\.py'$/      - '**.py'/g" "$filepath"
            sed -i '' "s/^  #     - 'pytest\.ini'$/      - 'pytest.ini'/g" "$filepath"
            sed -i '' "s|^  #     - 'docker/\*\*'$|      - 'docker/**'|g" "$filepath"
        else
            # Linux sed
            sed -i 's/^  # push:  # DISABLED FOR PRIVATE REPO - enabled in public repo$/  push:/g' "$filepath"
            sed -i 's/^  #   branches:$/    branches:/g' "$filepath"
        ...
        log_success "Enabled push trigger in $DOCKER_WORKFLOW"
    else
        log_warn "File not found: $DOCKER_WORKFLOW"
    fi
}
...
Workflows with enabled push trigger:
  - $DOCKER_WORKFLOW
"
}
