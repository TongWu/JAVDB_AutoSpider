...
remove_excluded_files() {
    log_info "Removing excluded files..."
    
    for pattern in "${EXCLUDE_PATTERNS[@]}"; do
        if [ -e "$PROJECT_ROOT/$pattern" ]; then
            log_info "Removing: $pattern"
            git rm -rf --cached "$PROJECT_ROOT/$pattern" 2>/dev/null || true
...
- Removed config.py with sensitive data
...
push_to_publish() {
    log_info "Pushing to origin/$PUBLISH_BRANCH..."
    
    git push -f origin "$PUBLISH_BRANCH"
    log_success "Pushed to origin/$PUBLISH_BRANCH"
}
...
Options:
  ...
  --https            Use HTTPS URL (reads auth from config.py)

Examples:
  # Use config.py settings (recommended)
  $0

  # Use SSH mode (requires SSH key)
  $0 --ssh

  # Manually specify public repo URL
  $0 git@github.com:username/public-repo.git
...
main() {
    ...
    local use_ssh=true  # Default to SSH
    ...
    # Setup public repository remote
    setup_public_remote "$public_url" "$use_ssh"
