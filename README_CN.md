...
完整安装指南请参阅 [本地安装指南](docs/handbook/zh/self-hoster/local-setup.md)。
...
规范布局由 [ADR-007](docs/design/_archive/ADR-007-Monorepo-Restructure/ADR-007-monorepo-restructure-2026-05.zh.md) 确立；所有旧路径（`utils/`、`api/`、`migration/`、`legacy/`、`scripts/spider/`、`scripts/ingestion/`、根目录 `compat.py`/`pipeline.py`）已在 Phase 3 移除。
...
完整配置参考（60+ 选项），请参阅 [配置指南](docs/handbook/zh/self-hoster/configuration.md)。
...
| `WRITE_MODE` | `pending` | 仅支持 `pending`；遗留 `audit` 请求会降级为 pending |
...
| `D1_RECOVERY_OUTBOX_ENABLED` | 未设置 | ADR-010 Phase 2 开关；设为 `1` 时，安全 D1 写失败可进入 `reports/D1/d1_recovery_outbox.jsonl`；D1 模式仍失败，dual 模式会在恢复清空前阻止提交。 |
...
| `D1_STARTUP_REPLAY_ENABLED` | 未设置 | ADR-010 Phase 4 开关；设为 `1` 时，进程首次打开 D1 或 Dual 连接会清空非 dead-lettered 恢复工作。 |
...
完整 CLI 参考请参阅 [CLI 参考文档](docs/handbook/zh/developer/cli-reference.md)。

## 部署方式

| 方式 | 指南 | 适用场景 |
|------|------|----------|
| **本地** | [本地安装](docs/handbook/zh/self-hoster/local-setup.md) | 开发、手动运行 |
| **GitHub Actions** | [GH Actions 部署](docs/handbook/zh/self-hoster/github-actions-setup.md) | 自动化日常流水线 |
| **Docker** | [Docker 部署](docs/handbook/zh/self-hoster/docker-deploy.md) | 自托管服务器 |
| **代理协调器** | [代理协调器部署](docs/handbook/zh/self-hoster/proxy-coordinator.md) | 多 Runner 协调 |
...
回滚操作请参阅 [D1 回滚指南](docs/handbook/zh/ops/d1-rollback.md)。

## 文档

### 自部署者
- [本地安装](docs/handbook/zh/self-hoster/local-setup.md) — 从零开始安装
- [GitHub Actions 部署](docs/handbook/zh/self-hoster/github-actions-setup.md) — CI/CD 部署
- [Docker 部署](docs/handbook/zh/self-hoster/docker-deploy.md) — 容器部署
- [配置参考](docs/handbook/zh/self-hoster/configuration.md) — 全部 60+ 配置选项
- [代理协调器](docs/handbook/zh/self-hoster/proxy-coordinator.md) — 跨 Runner 协调
- [代理设置](docs/handbook/zh/self-hoster/proxy-setup.md) — 代理池配置
- [CloudFlare 绕过](docs/handbook/zh/self-hoster/cloudflare-bypass.md) — CF 挑战回退
- [JavDB 登录](docs/handbook/zh/self-hoster/javdb-login.md) — 会话 cookie 刷新
- [Web UI 部署](docs/handbook/zh/self-hoster/web-ui-deploy.md) — Web UI 与 API 栈
- [Rust 安装](docs/handbook/zh/self-hoster/rust-installation.md) — 可选 Rust 扩展

### 开发者
- [CLI 参考](docs/handbook/zh/developer/cli-reference.md) — 所有 CLI 命令和参数
- [API 使用指南](docs/handbook/zh/developer/api-usage-guide.md) — Python 模块和 REST API
- [历史系统](docs/handbook/zh/developer/history-system.md) — 重复预防和追踪

### 运维
- [D1 回滚](docs/handbook/zh/ops/d1-rollback.md) — 回滚 SOP 和派发矩阵
...
- [JavDB 登录指南](docs/handbook/zh/self-hoster/javdb-login.md) — 登录故障排查
