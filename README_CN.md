...
- **D1 / SQLite 回滚**：每次运行都会被打上 `ReportSessions.Id`（51 位的应用层雪花 id，不再依赖 AUTOINCREMENT，详见下方 "SessionId 生成策略"）以及由 `GITHUB_RUN_ID` / `GITHUB_RUN_ATTEMPT` 派生的 `(RunId, RunAttempt)` 二元组，并维护生命周期状态（`in_progress` → `committed`/`failed`）。失败的流水线会自动触发 `cleanup-on-failure` job，用 audit 表回放 + 每会话 staging 切换的方式撤销未提交的写入。手工恢复走 [`.github/workflows/RollbackD1.yml`](.github/workflows/RollbackD1.yml)；每天的 [`.github/workflows/StaleSessionCleanup.yml`](.github/workflows/StaleSessionCleanup.yml) cron 还会清理任何停留 `in_progress` 超过 48h 的会话。完整 SOP、"重试失败 job" 安全矩阵与 audit 表诊断路径见 [docs/D1_ROLLBACK.md](docs/D1_ROLLBACK.md)。
