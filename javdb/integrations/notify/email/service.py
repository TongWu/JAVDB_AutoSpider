...
The individual responsibilities live in sibling submodules:
...
This module has NO command-line surface: the canonical command adapter is
:mod:`apps.cli.notify.email`, which maps parsed arguments to
:class:`EmailNotificationOptions` and calls :func:`run_email_notification`. The
read/format/delivery helpers are imported into this module's namespace so they
resolve as ``service``-module globals (tests monkeypatch them at
``javdb.integrations.notify.email.service.<name>``).
"""
...
# tests that monkeypatch this module continue to work).
...
def run_email_notification(
    ...
    Analyses logs, loads stats from the SQLite-local mirror, builds the report
    body / subject, sends (or dry-run-fingerprints) the email, cleans up
