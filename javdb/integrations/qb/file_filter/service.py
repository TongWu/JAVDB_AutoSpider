#!/usr/bin/env python3
"""qBittorrent File Filter service.

Filters out small files from recently added torrents in qBittorrent by setting
the download priority to 0 (do not download) for files below the threshold.
Optionally deletes local files that have already been downloaded.

CLI parsing and exit-code mapping live in ``apps.cli.qb.file_filter``; this
module is the pure service layer.
"""
...
def run_file_filter_api(
    ...
    Orchestrates the same steps as the CLI service but returns a structured
    dict instead of an exit code. Designed to be called from the REST API
    handler (``apps/api/routers/operations.py``) with its integration points
    patchable via ``unittest.mock.patch``. Exposed at the package level as
    ``javdb.integrations.qb.file_filter.run_file_filter`` for backwards
    compatibility with that caller and its tests.
...
def run_file_filter_cli(options: QbFileFilterOptions) -> QbFileFilterResult:
    ...
    calling ``sys.exit`` it returns a :class:`QbFileFilterResult` whose
    ...
    # ``--categories`` JSON parsing now lives in the CLI adapter; the service
    # receives the already-parsed list.
    categories_list = list(options.categories) if options.categories else None
    if categories_list:
        logger.info(f"Configuration: min_size={options.min_size_mb}MB, days={options.days}, categories={categories_list}, dry_run={options.dry_run}, delete_local_files={options.delete_local_files}")
    else:
        logger.info(f"Configuration: min_size={options.min_size_mb}MB, days={options.days}, category={options.category}, dry_run={options.dry_run}, delete_local_files={options.delete_local_files}")
