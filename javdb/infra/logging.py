...
_MODULE_SHORT_NAMES = {
    # javdb_platform
    'javdb.infra.request': 'RequestHandler',
    'javdb.proxy.pool': 'ProxyPool',
    'javdb.proxy.ban_manager': 'BanManager',
    'javdb.proxy.policy': 'ProxyPolicy',
    'javdb.infra.logging': 'LogConfig',
    'javdb.infra.config': 'Config',
    'javdb.storage.history_manager': 'History',
    'javdb.pipeline.service': 'Pipeline',
    'javdb.storage.db.db': 'DB',
    'javdb.infra.csv_writer': 'CSVWriter',
    'javdb.infra.git_helper': 'Git',
    'javdb.spider.spider_gateway': 'Gateway',
    'javdb.infra.paths': 'PathHelper',
    'javdb.integrations.qb.config': 'QBConfig',
    # javdb_platform — coordinator / D1 clients (added in log redesign)
    'javdb.proxy.coordinator.runner_registry_client': 'RunnerRegistry',
    'javdb.proxy.coordinator.movie_claim_client': 'MovieClaim',
    'javdb.proxy.coordinator.proxy_coordinator_client': 'ProxyCoord',
    'javdb.proxy.coordinator.login_state_client': 'LoginState',
    'javdb.storage.d1_client': 'D1',
    'javdb.storage.dual_connection': 'DualDB',
    # javdb_spider
    'javdb.spider.fetch.fetch_engine': 'FetchEngine',
    'javdb.spider.fetch.index': 'IndexFetch',
    'javdb.spider.fetch.index_parallel': 'IndexFetch',
    'javdb.spider.fetch.fallback': 'Fallback',
    'javdb.spider.fetch.session': 'Session',
    'javdb.spider.fetch.login_coordinator': 'Login',
    'javdb.spider.fetch.sequential_backend': 'SeqBackend',
    'javdb.spider.detail.runner': 'DetailRunner',
    'javdb.spider.detail.parallel_mode': 'ParallelMode',
    'javdb.spider.runtime.sleep': 'SleepMgr',
    'javdb.spider.runtime.state': 'SpiderState',
    'javdb.spider.runtime.config': 'SpiderConfig',
    'javdb.spider.runtime.report': 'Report',
    'javdb.spider.app.run_service': 'Spider',
    'javdb.spider.services.dedup_query': 'Dedup',
    'javdb.spider.services.dedup_store': 'Dedup',
    # javdb_core
    ...
    'javdb.spider.magnet_extractor': 'MagnetExtractor',
    # javdb_integrations
    ...
    'javdb.integrations.rclone.dedup': 'RcloneHelper',
    'javdb.integrations.rclone.path_utils': 'RcloneHelper',
    'javdb.integrations.rclone.scan': 'RcloneHelper',
    'javdb.integrations.rclone.types': 'RcloneHelper',
    ...
    'javdb.infra.health_check': 'HealthCheck',
    'javdb.spider.auth.login': 'JavDBLogin',
    'javdb.infra.fetch_page': 'FetchPage',
    # javdb_ingestion
    'javdb.pipeline.adapters': 'Adapters',
    'javdb.pipeline.policies': 'Policies',
    # javdb_migrations
    'javdb.migrations.migrate_to_current': 'Migration',
    'javdb.migrations.tools.csv_to_sqlite': 'MigCSVtoSQLite',
    'javdb.migrations.tools.reconcile_d1_drift': 'D1Reconcile',
...
def _shorten_logger_name(name):
    ...
    # package (e.g. ``javdb.integrations.qb.uploader.service``,
    # ``javdb.integrations.notify.email.delivery``). Fall back to the immediate
    # parent package's short label so the display name survives the
    # integration→package split instead of rendering the full dotted path.
    parent = name.rpartition('.')[0]
    if parent and parent in _MODULE_SHORT_NAMES:
