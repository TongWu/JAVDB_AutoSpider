...
@dataclass
class ActorCredit:
    """One actor on the detail page (name, link, gender from female/male marker)."""
    name: str
    href: str
...
# Canonical DB/API value when the actor panel has no /actors/ links but shows a placeholder (e.g. N/A).
...
class MovieDetail:
    ...
    # True when actor panel has placeholder text (e.g. N/A) and no /actors/ links.
...
@dataclass
class TagCategory:
    """One filter category on the tag page (e.g. subject, role, body type ...).
