...
def test_commit_workflow_outputs_reports_push_error(monkeypatch):
    monkeypatch.setattr(git_side_effects, "_has_git_credentials", lambda username, password: True)
    monkeypatch.setattr(git_side_effects, "_flush_log_handlers", lambda: None)

    def _boom(**kwargs):
        raise RuntimeError("push rejected")

    monkeypatch.setattr(git_side_effects, "_git_commit_and_push", _boom)
    ...
    assert result == GitCommitResult(committed=False, skipped_reason=None, error="push rejected")
