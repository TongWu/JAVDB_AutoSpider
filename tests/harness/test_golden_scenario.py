...
def test_golden_daily_run_writes_two_movies(pipeline_harness):
    ...
    # The spider fetched the index + both detail pages entirely from the
    # cassette — assert zero misses so a detail-page miss can't slip through.
    assert result.http.misses == []
