...
- **D1 / SQLite rollback (Phase 3 default = pending mode)**: every run is tagged with a `ReportSessions.Id` (a 51-bit application-generated snowflake id, *not* AUTOINCREMENT — see "SessionId generation" below) plus `WriteMode` (`pending` default, `audit` legacy fallback) plus a `(RunId, RunAttempt)` pair derived from `GITHUB_RUN_ID` / `GITHUB_RUN_ATTEMPT`, and a richer lifecycle status (`in_progress` → `finalizing` → `committed` / `failed`). Pending-mode history writes go through `PendingMovie/TorrentHistoryWrites` and only land in `MovieHistory` / `TorrentHistory` at commit time — **failed runs delete pending rows** rather than replaying audit. Failed pipelines auto-trigger `cleanup-on-failure`, which now dispatches per session: `in_progress` → rollback, `finalizing` → `db_resume_finalizing_session`. Manual recovery is available through [`.github/workflows/RollbackD1.yml`](.github/workflows/RollbackD1.yml); the daily [`.github/workflows/StaleSessionCleanup.yml`](.github/workflows/StaleSessionCleanup.yml) cron applies the same dispatch matrix to sessions stuck >48h. Email subjects auto-prefix `[PENDING-ALERT]` / `[PENDING-ROLLBACK-AUTO]` on threshold breach, and a critical alert writes `pending_mode_disabled_until` to `.publish-config.yml` to fall back to audit for 24h via [`scripts/pending_mode_auto_fallback.py`](scripts/pending_mode_auto_fallback.py). Full SOP, dispatch matrix, 6-step validation playbook, "Re-run failed jobs" safety matrix, and audit-table forensics: [docs/D1_ROLLBACK.md](docs/D1_ROLLBACK.md).
- **Phase 4 — Audit deprecation (2026-05-13)**: `db_upsert_history` now emits `DeprecationWarning`; the legacy `WriteMode='audit'` fallback path sunsets on **2026-08-13**. `MovieHistoryAudit` / `TorrentHistoryAudit` tables are read-only forensics — direct writes are gated by the new `JAVDB_AUDIT_WRITES_DISABLED` kill switch (defaults to off until every workflow stops pinning audit). The weekly [`.github/workflows/AuditArchive.yml`](.github/workflows/AuditArchive.yml) cron, backed by [`scripts/audit_archive.py`](scripts/audit_archive.py), prunes audit rows whose owning session is older than 30 days (committed, failed-expired, or orphan). The one-shot incident helper [`scripts/cleanup_stale_session_audits.py`](scripts/cleanup_stale_session_audits.py) is now strictly read-only — its `--apply` flag logs a deprecation warning and degrades to dry-run. See [docs/D1_ROLLBACK.md §Appendix A](docs/D1_ROLLBACK.md#appendix-a-legacy-audit-fallback-sunset-2026-08) for the full sunset timeline.
- **SessionId generation**: under `STORAGE_BACKEND=dual` the per-backend AUTOINCREMENT counters drift permanently after any single asymmetric INSERT (one side commits, the other fails), so the application generates `ReportSessions.Id` itself as `(time.time_ns() // 1_000_000) << 10` with a 10-bit per-process counter (`packages/python/javdb_platform/db.py::_generate_session_id`). The same id is INSERTed explicitly on both backends, eliminating the source of the 2026-05-08 incident. Audit rows additionally carry `(RunId, RunAttempt)` columns so the rollback CLI can address sessions by GitHub run identity even if a prior failed rollback deleted the owning `ReportSessions` row.
- **Strict dual-write (2026-05-11+)**: set `STRICT_DUAL_WRITE=1` to make any D1 write failure during a transaction fatal — `DualConnection.commit()` raises `DualWriteStrictError` when `d1_failure_count > 0`, forcing SQLite rollback and aborting the session instead of silently committing only-on-SQLite rows. `DualConnection.executemany` now chunks at `_BATCH_LIMIT` rows and applies D1 *before* SQLite per chunk so the canonical store can never get ahead of D1; chunk failures still log `partial_prefix_count` to `reports/D1/d1_drift.jsonl` so the reconciler ([`scripts/sync_d1_to_sqlite.py`](scripts/sync_d1_to_sqlite.py)) knows exactly how many rows D1 kept. The default (unset) preserves legacy "SQLite is canonical, drift is logged" behaviour; flip the flag in CI / staging first.
- **`sync_d1_to_sqlite.py` defaults to UPSERT (2026-05-11+)**: the apply path no longer `DELETE`s the local table before re-inserting from D1. It now `INSERT OR REPLACE`s row-by-row keyed on the primary key, preserving SQLite-only rows by default. Pass `--prune-local-only` (plus `--allow-local-prune-on-drift` if you really mean it) to opt into the legacy destructive prune.
- **Email notification SMTP failures exit non-zero (2026-05-11+)**: `email_notification.py` now exits with code `2` when SMTP send fails outside of `--dry-run`, so CI no longer silently marks an undelivered email as "notified". Under `STORAGE_BACKEND=dual` stats are now read from the local SQLite mirror (`db_get_*_stats_local` / `db_get_latest_session_local`) and the email body is prefixed with a `🚨 D1 DRIFT ADVISORY` banner whenever `reports/D1/d1_drift.jsonl` shows drift recorded today.
- **Rollback CLI lookup precedence** (most specific wins; sources are unioned): `--session-id` → `(--run-id, --attempt)` (queries both `ReportSessions` and the audit tables) → `--run-started-at` window scan **only** when `--include-orphaned` is set (the legacy "sweep this window" behaviour is now opt-in to avoid the 2026-05-08 mistake where a manual `--session-id 332` cleanup silently expanded to also clean session 346). The CLI also refuses any candidate session whose `DateTimeCreated` predates `--run-started-at` by more than one hour (cross-day reject, exit code 2).
