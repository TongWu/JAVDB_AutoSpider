...
- **D1 / SQLite rollback**: every run is tagged with a `ReportSessions.Id` (a 51-bit application-generated snowflake id, *not* AUTOINCREMENT — see "SessionId generation" below) plus a `(RunId, RunAttempt)` pair derived from `GITHUB_RUN_ID` / `GITHUB_RUN_ATTEMPT`, and lifecycle status (`in_progress` → `committed`/`failed`). Failed pipelines auto-trigger the `cleanup-on-failure` job that unwinds uncommitted writes via the audit-table replay + per-session staging swap. Manual recovery is available through [`.github/workflows/RollbackD1.yml`](.github/workflows/RollbackD1.yml). A daily [`.github/workflows/StaleSessionCleanup.yml`](.github/workflows/StaleSessionCleanup.yml) cron unwinds any session stuck `in_progress` for more than 48h. Full SOP, "Re-run failed jobs" safety matrix, and audit-table forensics: [docs/D1_ROLLBACK.md](docs/D1_ROLLBACK.md).
